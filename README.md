...
![Next.js](https://img.shields.io/badge/Next.js-16-black) ![TypeScript](https://img.shields.io/badge/TypeScript-5.9-blue) [![test](https://github.com/sangholabs/isa-lab/actions/workflows/test.yml/badge.svg)](https://github.com/sangholabs/isa-lab/actions/workflows/test.yml) ![license](https://img.shields.io/badge/license-MIT-green)
...
### 리서치 (선택)
자기 API 키를 넣으면 Gemini · Claude · GPT · Perplexity가 종목을 분석합니다. 결과에는 **신뢰도 점수**, **모델 간 기울기**, **판단 체크리스트**, **세후 시나리오**가 함께 나옵니다. 자세한 설계는 아래.

![리서치](docs/screenshot-research.png)
...
「이 종목 85점」을 띄우면 사람은 점수만 보고 근거를 안 읽습니다. 그러면 모델이 틀렸을 때 걸러낼 방법이 없습니다.
그래서 채점 대상을 뒤집었습니다 — **이 분석을 얼마나 믿을 수 있는가.**

| 항목 | 배점 | 무엇을 보나 |
|---|---|---|
| 근거 충실도 | 30 | 주장에 근거가 붙은 비율 |
| 출처 | 20 | 검증 가능한 링크가 있는가 |
| 논거 균형 | 20 | 한쪽만 쓰지 않았는가 |
| 불확실성 인정 | 15 | 모르는 걸 모른다고 적었는가 |
| 모델 자기평가 | 15 | 모델이 스스로 매긴 확실성 |

출처가 0건이거나 한쪽 논거만 있으면 **경고 문구가 따로 붙습니다.**

여기에 **모델이 만들지 않는 것** 두 가지를 더합니다.

- **판단 체크리스트** — 「이 돈을 얼마나 안 써도 되나」, 「얼마나 떨어지면 틀렸다고 인정할 건가」. 종목이 무엇이든 사람이 답해야 하는 질문이라 모델에게 맡기지 않았습니다.
- **세후 시나리오** — ±10/20/30%일 때 계좌별로 손에 남는 금액. LLM이 아니라 **세금 엔진이 계산한 결정론적 수치**입니다. 이 앱이 실제로 더할 수 있는 건 「얼마나 오를까」가 아니라 「그만큼 올랐을 때 뭐가 남나」입니다.
